...
export default function EconomicsSidebar({ currentLessonId, currentTopicId }: EconomicsSidebarProps) {
  ...
  return (
    ...
    >
      ...
      <div className="p-4 border-b relative">
        {!isCollapsed ? (
          <h2 className="text-lg font-semibold">Economics Content</h2>
        ) : (
              ...
              <ul className={`mt-1 space-y-0.5 ${isCollapsed ? 'flex flex-col items-center' : ''}`}>
                {unit.topics.map((topic) => (
                  <li key={topic.id} className={isCollapsed ? 'w-full' : ''}>
                    <Link
                      href={`/tenth-standard/social/economics/lessons/${unit.id}/${topic.id}`}
