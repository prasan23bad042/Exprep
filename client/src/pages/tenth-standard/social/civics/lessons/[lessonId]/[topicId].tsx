...
import CivicsSidebar from '@/components/course/CivicsSidebar';
...
export default function CivicsTopicPage() {
    ...
    return (
      <div className="container mx-auto p-8">
        <Button asChild variant="ghost" className="mb-6">
          <Link href="/tenth-standard/social/civics">
  ...
  const unitIndex = civicsUnits.findIndex(u => u.id === lessonId);
  ...
  if (unit && topicIndex >= 0) {
    if (topicIndex > 0) {
      ...
      const prevUnit = civicsUnits[unitIndex - 1];
    ...
    if (topicIndex < unit.topics.length - 1) {
      ...
      nextUnitId = unit.id;
    } else if (unitIndex < civicsUnits.length - 1) {
      const nextUnit = civicsUnits[unitIndex + 1];
        ...
        <div className="lg:w-80 flex-shrink-0">
          <div className="sticky top-24 h-[calc(100vh-200px)]">
            <div className="sidebar-container pr-2">
              <CivicsSidebar currentLessonId={lessonId} currentTopicId={topicId} />
            </div>
          </div>
        </div>
              ...
              <div className="mt-8 pt-4 border-t flex justify-between">
                {previousTopic ? (
                  <Button asChild variant="outline">
                    <Link href={`/tenth-standard/social/civics/lessons/${previousUnitId}/${previousTopic.id}`}>
                ...
                {nextTopic ? (
                  <Button asChild>
                    <Link href={`/tenth-standard/social/civics/lessons/${nextUnitId}/${nextTopic.id}`}>
                      {nextTopic.title}
